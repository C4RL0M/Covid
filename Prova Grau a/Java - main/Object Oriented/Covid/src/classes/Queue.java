...
public class Queue {
    ...
    public Patient getPatient(String name)throws CarlosQueueException{
        for (Patient obj : patients) {
            if (obj.getName() == name) {
                return obj;
            }
        }
        throw new CarlosQueueException("Paciente não encontrado");
    }
    ...
    public Patient nextPatient()throws CarlosQueueException {
        if (!patients.isEmpty()) {
            return patients.get(0);
        }
        throw new CarlosQueueException("Nenhum paciente cadastrado");
    }
